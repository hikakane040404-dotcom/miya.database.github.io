...
async function fetchCiNii(query) {
    ...
    // CiNii Research (より新しく安定したAPI) に切り替え
    const baseUrl = `https://cir.nii.ac.jp/opensearch/all?q=${encodeURIComponent(query)}&format=json`;
    ...
    try {
        ...
        if (!text.trim().startsWith('{') && !text.trim().startsWith('[')) {
            console.warn('GAS returned invalid format. Falling back to direct fetch.');
            return fetchCiNiiDirect(query);
        }

        const data = JSON.parse(text);
        const items = data['items'] || data['@graph']?.[0]?.['items'] || [];
        return items.map(item => parseCiNiiItem(item));
    } catch (e) {
        console.warn('CiNii (GAS proxy) fetch failed, trying direct fallback...', e);
        return fetchCiNiiDirect(query);
    }
}
...
async function fetchCiNiiDirect(query) {
    return new Promise((resolve) => {
        ...
        script.src = `https://cir.nii.ac.jp/opensearch/all?q=${encodeURIComponent(query)}&format=json&callback=${callbackName}`;
...
function parseCiNiiItem(item) {
    ...
    // 日付の取得を強化
    const pubDate = item['dc:date'] || item['prism:publicationDate'] || '';
    const year = pubDate.toString().substring(0, 4) || '不明';

    return {
        source: 'CiNii',
        type: 'journal',
        title: title,
        author: String(author).replace(/[\s　]+/g, '').replace(/[著編]/g, ''),
        year: year,
